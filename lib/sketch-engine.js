...
import { unique, SERVER, sketchEngineProxy } from '@/lib/utils'
...
export default {
  ...
  async getCorpora() {
    if (typeof this.corpora === 'undefined') {
        if (!this.gettingCorpora) {
            this.gettingCorpora = true;
            let url = `${SERVER}data/sketch-engine/sketch-engine-corpora.json.txt`;
            try {
        ...
        return new Promise((resolve, reject) => {
            const checkGettingCorpora = setInterval(() => {
                if (!this.gettingCorpora) {
                    clearInterval(checkGettingCorpora);
                    resolve(this.corpora);
                }
  ...
  async concordance(options) {
    ...
    const corpora = await this.getCorpora();
    if (!corpora) return;

    const corpname = options.corpname;
    if (!corpname) return;

    const corpus = corpora.find(corpus => corpus.corpname === corpname);
    if (!corpus) return false;

    const parallel = corpus.aligned && corpus.aligned.length > 0;
    const { concordanceQuery, concordanceQueryParallel } = this.buildConcordanceQuery(options, parallel);

    const requestJSON = this.buildConcordanceRequestJSON(options, parallel, concordanceQuery, concordanceQueryParallel);
    ...
    try {
      const response = await axios.post(
        `${sketchEngineProxy}?https://app.sketchengine.eu/bonito/run.cgi/concordance?corpname=${corpname}`, 
        bodyFormData
      );
      return this.processConcordanceResponseData(response, options);
    } catch (err) {
  ...
  buildConcordanceQuery(options, parallel) {
    let concordanceQuery, concordanceQueryParallel;

    if (options.searchAsPhrase) {
      concordanceQuery = `[{"queryselector":"phraserow","phrase":"${options.term}","qmcase":false}]`;
      concordanceQueryParallel = concordanceQuery;
    } else {
      concordanceQuery = `[{"queryselector":"iqueryrow","sel_aligned":["opus2_${options.l1}"],"cql":"","iquery":"${options.term}","queryselector_opus2_${options.l1}":"iqueryrow","iquery_opus2_${options.l1}":"","pcq_pos_neg_opus2_${options.l1}":"pos","filter_nonempty_opus2_${options.l1}":"on"}]`;
      concordanceQueryParallel = `[{"queryselector":"iqueryrow","iquery":"${options.term}"}]`;
    }

    return { concordanceQuery, concordanceQueryParallel };
  },

  buildConcordanceRequestJSON(options, parallel, concordanceQuery, concordanceQueryParallel) {
    return parallel
      ? `{"attrs":"word","structs":"s,g","refs":"=doc.subcorpus","ctxattrs":"word","viewmode":"align","usesubcorp":"","freqml":[{"attr":"word","ctx":"0","base":"kwic"}],"fromp":${options.searchAsPhrase ? 2 : 1},"pagesize":1000,"concordance_query":${concordanceQuery}}`
      : `{"lpos":"","wpos":"","default_attr":"word","attrs":"word","refs":"=doc.website","ctxattrs":"word","attr_allpos":"all","usesubcorp":"","viewmode":"kwic","cup_hl":"q","cup_err":"true","cup_corr":"","cup_err_code":"true","structs":"s,g","gdex_enabled":0,"fromp":${options.searchAsPhrase ? 2 : 1},"pagesize":50,"concordance_query":${concordanceQueryParallel},"kwicleftctx":"100#","kwicrightctx":"100#"}`;
  },

  processConcordanceResponseData(response, options) {
    try {
      ...
      if (data.Lines && data.Lines.length > 0) {
        for (let Line of data.Lines.slice(0, 500)) {
          const line = this.buildLine(Line);
          if (line.length > options.term.length + 4) {
            let parallelLine = this.buildParallelLine(Line, line);
            result.push(parallelLine);
          }
        }
        result = result.sort((a, b) => a.l2.length - b.l2.length);
      }
  ...
  buildLine(Line) {
    return Line.Left.map(item => (item ? item.str : '')).join(' ') +
      ' ' +
      Line.Kwic[0].str +
      ' ' +
  ...
  async mistakes(options) {
    ...
    let response = await axios.post(
      `${sketchEngineProxy}?https://app.sketchengine.eu/bonito/run.cgi/concordance?corpname=preloaded/guangwai`,
      bodyFormData)
    ...
    if (data.Lines && data.Lines.length > 0) {
      for (let Line of data.Lines) {
        ...
        const countryCode = refs['=text.id'].replace(
          /^[^_]*_[^_]*_[^_]*_[^_]*_([^_]*).*/g,
          '$1'
        )
        results.push({
