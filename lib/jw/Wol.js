...
import { proxy, proxyParsed, unlessUndefined, getTodaysDate } from '@/lib/utils'

export const WOL_BASE_URL = 'https://wol.jw.org/'
export const DATA_UPLOADS_URL = 'https://study.voxelcreative.ca/wp-content/uploads/jw-study-aid/'
export const VIDEO_SCRIPTURE_SEARCH_BASE = 'https://study.voxelcreative.cawp-json/videosearch/scripture?'
export const VIDEO_TERM_SEARCH_BASE = 'https://study.voxelcreative.ca/wp-json/videosearch/search?'

export default {
  baseUrl: WOL_BASE_URL, // Compatibility
  WOL_BASE_URL,          // Compatibility
  async getLanguages() {
    /*
     {
     ...
     */
    let data = await proxy(`https://wol.jw.org/wol/li`)
    if (data) {
      return data.items
    }
  },
  ...
  async getBibleBookGroups(l2) {
    let root = await proxyParsed(`https://wol.jw.org/${l2.wol.locale}/wol/binav/${l2.wol.libs[0].researchConfigurationID}/${l2.wol.libs[0].symbol}/nwt`)
    let audios = await proxy(`https://wol.jw.org/wol/baa/${l2.wol.libs[0].researchConfigurationID}/${l2.wol.libs[0].symbol}/nwt`)
    return [{
  ...
  async getBibleBook(id, l2) {
    let root = await proxyParsed(`https://wol.jw.org/${l2.wol.locale}/wol/binav/${l2.wol.libs[0].researchConfigurationID}/${l2.wol.libs[0].symbol}/nwt/${id}`)
    let chapters = []
  ...
  async getArticle(url, options) {
    ...
    let root = await proxyParsed(url + (snippet ? '?snip=yes' : ''))
    ...
    if (articleNode) {
      ...
      let article = {
        ...
        prevUrl: unlessUndefined(root.querySelector('.resultNavLeft a'), a => a.getAttribute('href')),
        nextUrl: unlessUndefined(root.querySelector('.resultNavRight a'), a => a.getAttribute('href')),
        contentsUrl: unlessUndefined(root.querySelector('.navPublications a'), a => a.getAttribute('href')),
        title: unlessUndefined(root.querySelector('#contentTitle'), t => t.getAttribute('value')) || unlessUndefined(root.querySelector('.resultDocumentPubTitle'), t => t.innerText.trim()),
        publicationTitle: unlessUndefined(root.querySelector('#documentDescription') || root.querySelector('#parentTitle'), d => d.getAttribute('value')),
        thumbnail: unlessUndefined(root.querySelector('#contentThumbnail'), d => 'https://wol.jw.org' + d.getAttribute('value')),
        symbol: unlessUndefined(root.querySelector('#englishSym'), d => d.getAttribute('value')),
        audioSymbol: unlessUndefined(root.querySelector('#audioPubSym'), d => d.getAttribute('value')),
        docId: unlessUndefined(root.querySelector('#docId'), d => d.getAttribute('value')),
        pubMediaServer: unlessUndefined(root.querySelector('#pubMediaServer'), d => d.getAttribute('value')),
  ...
  getDayTextUrl(langData) {
    return this.baseUrl + langData.code + '/' + langData.day_text_url + getTodaysDate()
  },
  ...
  /**
   * Example: https://data.jw-api.org/mediator/v1/media-items/E/pub-mwbv_202205_3_VIDEO
   * @param {*} languageAgnosticNaturalKey example: `pub-mwbv_202205_3_VIDEO`
   * @param {*} callback 
   * @param {*} jwLangSymbol example: `CHS`
   */
  async getVideo(languageAgnosticNaturalKey, jwLangSymbol = langData.jwLangSymbol) {
    let url = 'https://data.jw-api.org/mediator/v1/media-items/' + langData.jwLangSymbol + '/' + languageAgnosticNaturalKey + '?clientType=jworg'
    let data = await proxy(url)
    if (data?.media?.[0]) return data.media[0]
  },
