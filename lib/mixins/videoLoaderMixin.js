...
export default {
  methods: {
    ...
    async getVideoFromDB(youtube_id, directus_id) {
      ...
      if (video) {
        // Only parse subs_l2 from the database, ignore subs_l1 completely
        for (let field of ["subs_l2"]) {
        ...
        // Ensure subs_l1 is always undefined so a fresh translation runs
        delete video.subs_l1;
        ...
        if (videoData) {
          properties.forEach((property) => {
            if (!video[property] && videoData[property]) {
              // Use your Vuex mutation instead of Vue.set directly on the object
              this.$store.commit("shows/MODIFY_ITEM", { 
                item: video, 
                key: property, 
                value: videoData[property] 
              });
            }
          });
        }
