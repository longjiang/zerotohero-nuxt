...
export default {
  methods: {
    ...
    async getVideoFromDB(youtube_id, directus_id) {
      ...
      if (video) {
        // Only parse subs_l2 from the database; subs_l1 is never fetched or persisted
        if (video.subs_l2 && typeof video.subs_l2 === "string") {
          let savedSubs = this.$subs.parseSavedSubs(video.subs_l2);
          if (savedSubs) {
            video.subs_l2 = savedSubs.filter(
