...
import { proxy, PROXY_URL, imageUrl, savePhoto } from "@/lib/utils"

export default {
  savePhoto(word, url, callback) {
    axios.get(
      `${savePhoto}?id=${word.hskId}&word=${word.simplified
      }&url=${encodeURIComponent(url)}`,
    ).then(response => callback(response.data))
  },
  getPhoto(word, success, fail = () => { }) {
    let src = `${imageUrl}${word.hskId}-${word.simplified}.jpg`
  ...
  // strWord = "视频"
  async getWebImages(strWord, { proxy = `${PROXY_URL}?`, cacheLife = -1 } = {}) {
