...
export default {
  ...
  async searchWithLike(terms, lang, langId, filter, adminMode, excludeTerms, continua) {
    let showFilter = ''
    ...
    for (let term of terms) {
      ...
      let response = await $.getJSON(
        `${Config.wiki}items/youtube_videos?${subsFilter}${showFilter}&filter[title][ncontains]=Clip&filter[l2][eq]=${langId
  ...
  async searchSubs(terms, excludeTerms, lang = 'en', langId = 1824, adminMode = false, continua = true) {
    let hits = []
    hits = hits.concat(await this.searchWithLike(terms, lang, langId, 'tv_show', adminMode, excludeTerms, continua))
    if (hits.length < 20) {
      hits = hits.concat(await this.searchWithLike(terms, lang, langId, 'talk', adminMode, excludeTerms, continua))
    }
    if (hits.length < 20 && !['zh', 'en', 'ja', 'ru'].includes(lang)) {
      hits = hits.concat(await this.searchWithLike(terms, lang, langId, false, adminMode, excludeTerms, continua))
    }
    return hits
  },
