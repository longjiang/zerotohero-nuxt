import { proxy, unique, scrape, YOUTUBE_VIDEO_URL, LRC_SERVER, YOUTUBE_CHANNEL_PLAYLISTS_URL, YOUTUBE_PLAYLIST_URL } from "@/lib/utils";
...
export default {
  ...
  async getL1Transcript(originalVideo, $l1, $l2, $adminMode = false) {
    ...
    if (missingSubsL1) {
      await this.addTranscriptLocalesToVideo(
        video,
        $l1,
        $l2,
  ...
  async getTranscriptLocales(youtube_id, $l1, $l2, cacheLife = -1) {
    let tracks = await this.getSubsList(youtube_id, cacheLife);
    if (!tracks && cacheLife === -1) {
      await this.getSubsList(youtube_id, 0); // Try again with cache buster
    }
    if (!tracks) return null;
  
    let l2Locales = unique([$l2.code, ...$l2.locales, ...$l2.hostCountryLocales]);
    ...
    if (matchedLocales.length > 0) l1Locale = matchedLocales[0];
  
    console.log(
      `YouTube: Got video transcripts available in L2 locale of '${l2Locale}' (name: '${l2Name}') and L1 locale of '${l1Locale}'.`
    );
  ...
  async addTranscriptLocalesToVideo(video, $l1, $l2, cacheLife = -1) {
    let { l1Locale, l2Locale, l2Name } = await this.getTranscriptLocales(video.youtube_id, $l1, $l2, cacheLife);
    if (l2Locale) {
      video.hasSubs = true;
