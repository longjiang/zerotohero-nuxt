...
export default {
    ...
    if (response.data.items && response.data.items.length > 0) {
      let video = response.data.items.map(item => {
        return {
          ...
          date: this.parseDate(item.snippet.publishedAt),
          data: item
        }
      })[0]
      return video
  ...
  async playlistByApi(id, pageToken = false, cacheLife = -1) {
    let { playlistItems, nextPageToken, totalResults } = await this.playlistPageByApi(id, pageToken, cacheLife)
    if (nextPageToken) {
      ...
      if (response.data.items) {
        playlistItems = response.data.items.map(item => {
          return {
            ...
            date: this.parseDate(item.contentDetails.videoPublishedAt),
            data: item,
          }
        })
      }
  ...
  async searchWithLike(terms, lang, langId, filter, adminMode, excludeTerms, continua, limit = 100, limitToTVShow = false, exact = false) {
    let showFilter = ''
    ...
    for (let term of terms) {
      ...
      if (typeof response !== 'undefined' && response.data && response.data.length > 0) {
        ...
        hits = hits.concat(this.getHits(videos, terms, excludeTerms, continua, false, exact))
        if (hits.length > limit) break
      }
    }
    return hits
  },
  async searchSubs(terms, excludeTerms, lang = 'en', langId = 1824, adminMode = false, continua = true, limit = 100, limitToTVShow = false, exact = false) {
    let hits = []
    hits = hits.concat(await this.searchWithLike(terms, lang, langId, 'tv_show', adminMode, excludeTerms, continua, limit, limitToTVShow, exact))
    if (hits.length < limit && !limitToTVShow && hits.length < 10) {
      hits = hits.concat(await this.searchWithLike(terms, lang, langId, 'talk', adminMode, excludeTerms, continua, limit, limitToTVShow, exact))
    }
    if (hits.length < limit && ![].includes(lang) && !limitToTVShow && hits.length < 10) {
      hits = hits.concat(await this.searchWithLike(terms, lang, langId, false, adminMode, excludeTerms, continua, limit, limitToTVShow, exact))
    }
    return Helper.uniqueByValue(hits, 'id')
  },
  getHits(videos, terms, excludeTerms, continua = true, convertToSimplified = false, exact = false) {
    let seenYouTubeIds = []
    let hits = []
    for (let video of videos) {
      if (video && !seenYouTubeIds.includes(video.youtube_id)) {
        ...
        for (let index in video.subs_l2) {
          ...
          let passed = false
          if (exact) {
            let punctuations = Helper.characterClass('Punctuation')
            let regex = '^\s*(' + terms.map(t => Helper.escapeRegExp(t)).join('|') + ')\s*$'
            let segs = line
              .replace(new RegExp(`[${punctuations}]`, "g"), "\n")
              .split("\n")
            ...
            let boundary = continua ? '' : `(^|[^${Helper.characterClass('L')}]+)`
            let termsStr = terms.map(t => Helper.escapeRegExp(t)).join('|').replace(/\\\*/g, '.+').replace(/[_]/g, '.')
            let regexStr = `${boundary}(${termsStr})`
