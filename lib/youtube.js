...
export default {
  ...
  async videoByApi(id, cacheLife = -1) {
    ...
    if (response.data.items && response.data.items.length > 0) {
      let video = response.data.items.map(item => {
        let date = this.parseDate(item.snippet.publishedAt)
        return {
          youtube_id: id,
  ...
  async searchWithLike(terms, lang, langId, filter, adminMode, excludeTerms, continua, limit = 100, limitToTVShow = false, exact = false, apostrophe = false) {
    let showFilter = ''
    ...
    for (let term of terms) {
      ...
      if (typeof response !== 'undefined' && response.data && response.data.length > 0) {
        ...
        hits = hits.concat(this.getHits(videos, terms, excludeTerms, continua, false, exact, apostrophe))
        if (hits.length > limit) break
      }
    }
    return hits
  },
  async searchSubs(terms, excludeTerms, lang = 'en', langId = 1824, adminMode = false, continua = true, limit = 100, limitToTVShow = false, exact = false, apostrophe = false) {
    let hits = []
    hits = hits.concat(await this.searchWithLike(terms, lang, langId, 'tv_show', adminMode, excludeTerms, continua, limit, limitToTVShow, exact, apostrophe))
    if (hits.length < limit && !limitToTVShow && hits.length < 10) {
      hits = hits.concat(await this.searchWithLike(terms, lang, langId, 'talk', adminMode, excludeTerms, continua, limit, limitToTVShow, exact, apostrophe))
    }
    if (hits.length < limit && ![].includes(lang) && !limitToTVShow && hits.length < 10) {
      hits = hits.concat(await this.searchWithLike(terms, lang, langId, false, adminMode, excludeTerms, continua, limit, limitToTVShow, exact, apostrophe))
    }
  ...
  getHits(videos, terms, excludeTerms, continua = true, convertToSimplified = false, exact = false, apostrophe = false) {
    ...
    if (exact) {
      punctuations = Helper.characterClass(apostrophe ? 'PunctuationNoApostrophe' : 'Punctuation')
      if (continua) {
