    ...
    for (let video of videos) {
      if (video && !seenYouTubeIds.includes(video.youtube_id)) {
        ...
        for (let index in video.subs_l2) {
          ...
          if (exact) {
            let punctuations = "。！？；：!?;:,."
            let regex = '^\s*(' + terms.map(t => Helper.escapeRegExp(t)).join('|').replace(/[*]/g, '.+').replace(/[_]/g, '.') + ')\s*$'
            let segs = line
            ...
            for (let seg of segs) {
              let test = new RegExp(
                ...
                seg)
              if (test) passed = true
              if (line.includes('Een lot uit de loterij?')) {
            ...
            let regex = boundary + '(' + terms.map(t => Helper.escapeRegExp(t)).join('|').replace(/[*]/g, '.+').replace(/[_]/g, '.') + ')'
            passed = new RegExp(
              regex, 'i'
            ).test(
              line +
              ...
              (excludeTerms.length === 0 ||
                !new RegExp(excludeTerms.map(t => Helper.escapeRegExp(t)).join('|')).test(
                  line
                ))
          }
          if (
