...
export default {
  ...
  async searchWithLike(terms, lang, langId, filter, adminMode, excludeTerms, continua, limit = 20, limitToTVShow = false, exact = false, apostrophe = false, convertToSimplified = false) {
    let showFilter = ''
  ...
  async searchSubs(terms, excludeTerms, lang = 'en', langId = 1824, adminMode = false, continua = true, limit = 20, limitToTVShow = false, exact = false, apostrophe = false, convertToSimplified = false) {
    let hits = []
    hits = hits.concat(await this.searchWithLike(terms, lang, langId, 'tv_show', adminMode, excludeTerms, continua, limit, limitToTVShow, exact, apostrophe, convertToSimplified))
    if (hits.length < limit && !limitToTVShow && hits.length < 10) {
      hits = hits.concat(await this.searchWithLike(terms, lang, langId, 'talk', adminMode, excludeTerms, continua, limit, limitToTVShow, exact, apostrophe, convertToSimplified))
    }
    if (hits.length < limit && ![].includes(lang) && !limitToTVShow && hits.length < 10) {
      hits = hits.concat(await this.searchWithLike(terms, lang, langId, false, adminMode, excludeTerms, continua, limit, limitToTVShow, exact, apostrophe, convertToSimplified))
    }
