...
export default {
  ...
  async searchSubs(terms, excludeTerms, lang = 'en', langId = 1824, adminMode = false, continua = true) {
    ...
    if (typeof Config.approvedChannels[lang] !== 'undefined' && Config.approvedChannels[lang].length > 0) {
      // or searchWithRegex
      hits = hits.concat(await this.searchWithRegex(terms, lang, langId, Config.approvedChannels[lang], adminMode, excludeTerms, continua))
      if (hits.length < 20 && Config.talkChannels[lang]) {
        hits = hits.concat(await this.searchWithRegex(terms, lang, langId, Config.talkChannels[lang], adminMode, excludeTerms, continua))
      }
    } else {
      hits = hits.concat(await this.searchWithRegex(terms, lang, langId, [], adminMode, excludeTerms, continua))
    }
    return hits
  },
