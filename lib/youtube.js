...
export default {
  ...
  async searchWithRegex(
    ...
    try {
      let response = await $.getJSON(
        `${Config.youtubeVideosTableName(langId)}?subs_l2=${terms.join(
          "|"
  ...
  async reportUnavailableVideo({ youtube_id, video_id, l2Code }) {
    try {
      if (youtube_id) {
        // Log it
        let payload = { youtube_id };
        if (id) payload.video_id = video_id;
        if (l2Code) payload.l2 = l2Code;
  ...
  async searchSubs({
    ...
    if (
      ...
      ![].includes(lang) &&
      hits.length < 30 &&
      tvShowFilter === "all" && talkFilter === "all"
    ) {
      hits = hits.concat(
        await this.searchWithLike({
          terms,
