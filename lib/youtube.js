...
export default {
  ...
  async getRandomEpisodeYouTubeId(langCode, langId) {
    let response = await axios.get(`${Config.wiki}items/youtube_videos?filter[channel_id][in]=${Config.approvedChannels[langCode]}&filter[title][ncontains]=Clip&filter[l2][eq]=${langId}&fields=youtube_id&limit=5000`)
    if (response.data) {
      response = response.data
