...
export default {
  ...
  async getTranslatedTranscript({
    ...
    let nameQuery = name ? `&name=${encodeURIComponent(name)}` : "";
    let generatedQuery = generated ? `&generated=true` : ""; // Whether or not to use auto-generated captions
    let url = `${PYTHON_SERVER}timedtext?v=${youtube_id}&lang=${locale}${nameQuery}&tlang=${tlang}&fmt=srv3${generatedQuery}`
    let xml = await proxy(
      url,
      { cacheLife: forceRefresh ? 0 : -1 }
    );
  ...
  detectYouTubeEntity(str) {
    ...
    const directPlaylistIdRegex = /^PL[A-Za-z0-9_-]{32}$/;
    ...
    if (str.length === 11) {
      youtube_id = str;
    } else if (directPlaylistIdRegex.test(str)) {
      playlist_id = str;
    }

    const videoMatch = str.match(videoIdRegex);
    if (videoMatch && videoMatch[2].length === 11) {
      youtube_id = videoMatch[2];
    }

    const playlistMatch = str.match(playlistIdRegex);
