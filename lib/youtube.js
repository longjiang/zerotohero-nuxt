...
export default {
  ...
  async getRandomEpisodeYouTubeId(langCode, langId) {
    let channelFilter = Config.approvedChannels[langCode] ? `&filter[channel_id][in]=${Config.approvedChannels[langCode]}` : ''
    let response = await axios.get(`${Config.wiki}items/youtube_videos?filter[title][ncontains]=Clip${channelFilter}&filter[l2][eq]=${langId}&fields=youtube_id&limit=5000`)
  ...
  async searchWithLike(terms, lang, langId, approvedChannels, adminMode, excludeTerms, continua) {
    let channelFilter = ''
    if (typeof approvedChannels !== 'undefined' && approvedChannels.length > 0) {
  ...
  async searchSubs(terms, excludeTerms, lang = 'en', langId = 1824, adminMode = false, continua = true) {
    ...
    if (typeof Config.approvedChannels[lang] !== 'undefined' && Config.approvedChannels[lang].length > 0) {
      // or searchWithRegex
      hits = hits.concat(await this.searchWithLike(terms, lang, langId, Config.approvedChannels[lang], adminMode, excludeTerms, continua))
      if (hits.length < 20 && Config.talkChannels[lang]) {
        hits = hits.concat(await this.searchWithLike(terms, lang, langId, Config.talkChannels[lang], adminMode, excludeTerms, continua))
      }
    } else {
      hits = hits.concat(await this.searchWithLike(terms, lang, langId, [], adminMode, excludeTerms, continua))
    }
    return hits
  },
  ...
  parseSavedSubs(jsonOrCSV) {
    return (jsonOrCSV.charAt(0) === '[') ? JSON.parse(jsonOrCSV) : Papa.parse(jsonOrCSV, { header: true }).data
  }
}
