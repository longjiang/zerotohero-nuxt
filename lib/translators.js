import { PYTHON_SERVER, LANGS_WITH_AZURE_TRANSLATE } from './utils'
...
zu	zul`,
  ...
  async translateWithBing({ text, l1Code, l2Code, context }) {
    // Remove initial dashes (one or multiple of them), and add it back if we find it
    // This is because the translator server for some reason returns a 418 I'm a teapot error if the text starts with a dash
    let initialDashes = '';
    
    // Check if the text starts with a dash and capture it
    ...
    let res = await axios.post(`${PYTHON_SERVER}/translate`, payload)
    if (res?.data?.translated_text) return initialDashes + res.data.translated_text;
  },


  async translateArrayWithBing({ textArray, l1Code, l2Code }) {
    
    if (!LANGS_WITH_AZURE_TRANSLATE.includes(l2Code)) {
      console.error(`Azure Translator does not support language code '${l2Code}'.`)
    }

    // Map 'no' to 'nb' for Azure Translator
    if (l1Code === 'no') l1Code = 'nb'
    if (l2Code === 'no') l2Code = 'nb'
    
    // post to api
    let res = await axios.post(`${PYTHON_SERVER}translate_array`, {
      texts: textArray,
      l1: l1Code,
      l2: l2Code,
    })
    if (res?.data?.translated_texts) return res.data.translated_texts
  },
  
  // https://www.npmjs.com/package/iframe-translator
  async trnslateWithGoogleByIframe({ text, l1Code, l2Code }) {
    let translation;
