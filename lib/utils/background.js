...
export const DEFAULT_BACKGROUND_IMAGE = `/img/background-speed.jpg`
...
export const background = (l2 = undefined) => {
  ...
  if (backgroundsKeyWords[l2.code]) {
    ...
    backgroundImages[l2.code] = `${SERVER}data/img/backgrounds/bg-${l2.code}-${Math.ceil(
      Math.random() * 10
    )}.jpg`;
  else if (l2.han)
    backgroundImages[l2.code] = `${SERVER}data/img/backgrounds/bg-zh-${Math.ceil(Math.random() * 10)}.jpg`;
  ...
  else backgroundImages[l2.code] = `${SERVER}data/img/backgrounds/background-branch.jpg`;
