...
export default {
  ...
  async load() {
    ...
    this.words = this.uniqueByValues(words, ['head', 'hanja'])
  ...
  parseDictionaryCSV(data) {
    ...
    words = words.filter(w => w.word.length > 0) // filter empty rows
      .map(item => {
        item.head = item.word
        item.search = item.head.toLowerCase()
        item.head = item.word
        delete item.word
  ...
  wordForms(word) {
    ...
    if (word.head.endsWith('다')) {
      let krForms = this.conjugate(word.head)
  ...
  lookupMultiple(text) {
    let words = this.words.filter(word => word && word.head === text)
    return words
  },
  random() {
  ...
  lookupFuzzy(text, limit = 30) {
    let words = []
    for (let word of this.words) {
      let search = word.head
      if (search && search.length > 0) {
        let distance = distance(search, text);
  ...
  lookupMultiple(text) {
    let words = this.words.filter(word => word && (word.head === text || word.hanja === text))
    return words
  },
