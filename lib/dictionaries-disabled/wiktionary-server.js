...
export default {
  ...
  lookup(text) {
    let word = this.words.find(word => word && word.head.toLowerCase() === text.toLowerCase())
    return word
  },
  lookupMultiple(text, ignoreAccents = false) {
    if (ignoreAccents && !this.accentCritical) text = this.stripAccents(text)
    let words = this.words.filter(word => word && word[ignoreAccents ? 'search' : 'head'].toLowerCase() === text.toLowerCase())
    return words
  },
  getWords() {
    return this.words
  ...
  getWordsThatContain(text) {
    let words = this.words.filter(w => w.head.includes(text))
    let strings = words
      .map((word) => word.head)
