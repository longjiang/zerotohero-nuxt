...
export default {
  ...
  mutuallyExclusive(a) {
    ...
    for (let item of a) {
      let pass = !a.find(j => {
        return j !== item && j.includes(item)
      })
      if (pass) mutuallyExclusive.push(item)
    }
  ...
  background(l2 = undefined) {
    let background
    if (typeof l2 === 'undefined') return `https://images.pexels.com/photos/2011367/pexels-photo-2011367.jpeg?auto=compress&cs=tinysrgb&dpr=3&h=1760&w=1280`;
    if (l2.code === 'ase') {
