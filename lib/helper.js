...
export default {
  ...
  uniqueSort(items, uniqueKey, sortKey, order, locale) {
    console.log(items, uniqueKey, sortKey, order, locale)
    return this.uniqueByValue(items, uniqueKey).sort((x, y) => {
      if (order === 'ascending') {
        return x[sortKey].localeCompare(y[sortKey], locale, { numeric: true })
      } else {
        return y[sortKey].localeCompare(x[sortKey], locale, { numeric: true })
      }
    })
