...
export default {
  backgroundImages: {
    default: `/img/background-earth-vector.jpg`
  },
  specialLanguages: {
    lzh: {
      name: "文言文",
  ...
  sampleText: {
    ...
    fr: "Entrez le texte français ici pour rechercher rapidement tous les mots.",
  ...
  logError(error, tag) {
    if (!tag) tag = 'Caller: ' + logError.caller
    if (error.response) {
  ...
  languageLevels(language) {
    ...
    for (let level in this.levels) {
      let exam = this.exams.find(e => e.lang === language.code && this.levels[level][e.slug]) || this.exams.find(e => e.slug === 'cefr')
      let key = exam.slug
      let levelNum = this.levels[level][key] || this.levels[level].cefr
      levels[level] = {
        exam,
  ...
  level(level, l2 = undefined) {
    let levels = this.languageLevels(l2)
    return levels[level]
  },
  ...
  background(l2 = undefined) {
    if (!l2) return this.backgroundImages.default
    if (this.backgroundImages[l2.code]) return this.backgroundImages[l2.code]
    ...
    if (backgroundsKeyWords[l2.code]) {
      this.backgroundImages[l2.code] = `https://source.unsplash.com/1600x900/?${backgroundsKeyWords[l2.code].replace(/\s/g, '+')}`
    }
    else if (["dz", "pau", "ps", "lzh", "zh", "ug", "bo", "non"].includes(l2.code))
      this.backgroundImages[l2.code] = `${Config.server}data/img/backgrounds/bg-${l2.code}-${Math.ceil(
        Math.random() * 10
      )}.jpg`;
    else if (l2.han)
      this.backgroundImages[l2.code] = `${Config.server}data/img/backgrounds/bg-zh-${Math.ceil(Math.random() * 10)}.jpg`;
    else if (l2.country && l2.country.length > 0) {
      let randomCountryName = l2.country[Math.floor(Math.random() * l2.country.length)].name
      this.backgroundImages[l2.code] = `https://source.unsplash.com/1600x900/?${randomCountryName}`;
    }
    else if (l2['iso639-1']) this.backgroundImages[l2.code] = `https://source.unsplash.com/1600x900/?${l2.name}`;
    else this.backgroundImages[l2.code] = `${Config.server}data/img/backgrounds/background-branch.jpg`;
    return this.backgroundImages[l2.code]
  },
  roundTo(n, dec = 2) {
