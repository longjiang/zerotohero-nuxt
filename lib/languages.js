...
const addDictionaryNameToL1 = (dictionaryName, l1Obj, l2_iso_639_3) => {
  // Ensure the dictionaries property exists on the language
  l1Obj.dictionaries = l1Obj.dictionaries || {};
  // Initialize or reuse the array for the target language (dictionary.l2)
  l1Obj.dictionaries[l2_iso_639_3] = l1Obj.dictionaries[l2_iso_639_3] || [];
  // Push the dictionary name (e.g., "freedict") to the array
  l1Obj.dictionaries[l2_iso_639_3].push(dictionaryName);
};

export default {
  fullyLoaded: false, // Whether the full list of thousands of languages are loaded (set by loadFull())
  l1s: [],
  youTubeLangs: LANGS_YOUTUBE_SUPPORTS,
  liveTVLangs: LANGS_WITH_LIVE_TV,
  contentLangs: LANGS_WITH_CONTENT,
  commonLangs: [],
  scriptsIndex: {},
  omniglotIndex: {},
  featureCache: {},
    ...
    if (!this.fullyLoaded) {
      ...
      let server =
        typeof window === "undefined"
          ? process.env.baseUrl
          : window.location.origin; // Use local (baseUrl defined in nuxt.config.js)
      let locales = this.loadCSVString(data.locales);
      let hours = this.loadCSVString(data.hours);
  ...
  exportCSV() {
    let languages = this.l1s.filter((obj) =>
      this.contentLangs.includes(obj.code)
    );
  ...
  addHostCountryLocales(l1, locales) {
    // Initialize an array to store the locales of the host country.
    let hostCountryLocales = [];

    // Get the locales of the first language (l1).
    let l1Locales = l1.locales;

    // If the primary locale of l1 is not in YouTube languages and l1 has iso639-3 code
    if (!this.youTubeLangs.includes(l1Locales[0]) && l1["iso639-3"]) {
      // Check if l1 has an associated country.
      if (l1.country && l1.country[0]) {
        // Iterate over each country associated with l1.
        for (let country of l1.country) {
          // If the country has languages, get the locales of its primary language.
          if (country.languages && country.languages[0]) {
            let primaryLanguageCode = country.languages[0];
            let primaryLanguageLocales = locales
              .filter((l) => l.code === primaryLanguageCode)
              .map((l) => l.locale);
            // Concatenate the locales to the hostCountryLocales array.
            hostCountryLocales.push(
              primaryLanguageCode,
              ...primaryLanguageLocales
            );
          }
          // Special cases: For Philippines, add "fil" and for Manx, use Irish ("ga").
          if (country.name === "Philippines") hostCountryLocales.push("fil");
          if (l1.code === "gv") hostCountryLocales = ["ga"];
        }
      }
    } else {
      // Define special cases where the fallback language is not the host country's primary language.
      let specialCases = {
        wo: ["fr"], // For Wolof, use French.
      };

      // Get the special case languages for l1.
      let specialCaseL2Codes = specialCases[l1Locales[0]];
      if (specialCaseL2Codes) {
        // If there are any special case languages, get their locales and add them to the hostCountryLocales array.
        for (let primaryLanguageCode of specialCaseL2Codes) {
          let primaryLanguageLocales = locales
            .filter((l) => l.code === primaryLanguageCode)
            .map((l) => l.locale);
          hostCountryLocales.push(
    ...
    // Remove duplicates from the hostCountryLocales array.
    hostCountryLocales = [...new Set(hostCountryLocales)];

    // Assign the processed host country locales back to l1.
    l1.hostCountryLocales = hostCountryLocales;
  },
  /**
   * Populates dictionary assignments to a list of languages (l1s).
   *
   * @param {Array} l1s - List of language objects to assign dictionaries to.
   * @param {Object} options - Configuration object.
   * @param {Array} options.dictionaries - List of dictionaries with mapping info.
   * @param {Array} options.wiktionary_mapped_langs - Additional languages to be mapped from wiktionary.
   */
  assignDictionaries(
    l1s,
    { dictionaries = [], wiktionary_mapped_langs = [] } = {}
  ) {
    // Loop through the dictionaries provided in options
    for (let dictionary of dictionaries) {
      const dictionaryName = dictionary.dictionary; // e.g. 'freedict'
      const dictionaryl1 = dictionary.l1; // e.g. 'eng'
      const dictinoaryL2 = dictionary.l2; // e.g. 'hun'

      // Find the matching language in l1s based on ISO 639-3 code
      let l1 = l1s.find((language) => language["iso639-3"] === dictionaryl1);

      // If a matching language is found
      if (typeof l1 !== "undefined") {
        addDictionaryNameToL1(dictionaryName, l1, dictinoaryL2);
      }
    }

    // Loop through the default wiktionary languages (data.wiktionary_langs), i.e. 'eng' and 'zho'
    for (let l1 in data.wiktionary_langs) {
      // Combine the default wiktionary languages with any additional ones from options
      let wiktionary_langs = data.wiktionary_langs[l1];
      let all_wiktionary_langs = wiktionary_langs;
      if (l1 === "eng") wiktionary_langs.concat(wiktionary_mapped_langs); // Only English has additional wiktionary languages

      // Find the corresponding language object in l1s
      let l1_object = l1s.find((l) => l["iso639-3"] === l1);

      // Assign "wiktionary-csv" to all the wiktionary languages for this l1_object
      ...
      // If the language is English, add some special assignments
      if (l1 === "eng") {
    ...
    // const englishLanguage = l1s.find((l) => l["iso639-3"] === "eng");

    // for (let l1_code of LANGS_WITH_AZURE_TRANSLATE) {
    //   const l1_obj = l1s.find((language) => language["iso639-1"] || language['iso639-3'] === l1_code); // e.g. Indonesian language object
    //   if (!l1_obj) {
    //     console.error('Could not find language with code "' + l1_code + '"');
    //     continue;
    //   }
    //   for (let l2_iso639_3 in englishLanguage.dictionaries) {
    //     // e.g. 'zho'
    //     const dictionaryNames = englishLanguage.dictionaries[l2_iso639_3]; // e.g. ['hsk-cedict', 'wiktionary-csv']
    //     for (let dictionaryName of dictionaryNames) {
    //       addDictionaryNameToL1(dictionaryName, l1_obj, l2_iso639_3); // e.g. 'hsk-cedict', indonesian language object, 'zho'
    //       console.log('Ading dictionary "' + dictionaryName + '" to "' + l1_obj.name + '" for "' + l2_iso639_3 + '"');
    //     }
    //   }
    // }
  },
  ...
  getFeatures({ l1, l2 }, browser) {
    ...
    if (
      ...
      LANGS_WITH_AZURE_TRANSLATE.includes(l1.code)
    ) {
  ...
  countryCode(l2) {
    // 1) 国旗を表示しない言語（人工言語・超国家的補助語など）
    const noFlagLanguages = new Set([
      'eo',   // Esperanto
      'tlh',  // Klingon
      'sjn',  // Sindarin
      'vo',   // Volapük
    ...
    if (noFlagLanguages.has(l2.code)) return null;

    // 2) 手動で確定した言語 → 国コード対応表（ISO 3166‑1 α2）
    const manuallySetCountries = {
      // 既存ペア
      af: 'ZA', az: 'AZ', ar: 'SA', en: 'GB', eu: 'ES', da: 'DK',
      gl: 'ES', pa: 'IN', ta: 'LK', hak: 'CN', bn: 'BD', yue: 'HK',
      ca: 'ES', nan: 'CN', nn: 'NO', ko: 'KR', he: 'IL', hi: 'IN',
      // 修正・追加分（主要居住国優先／ご指摘準拠）
      ur:  'PK', sli: 'PL', ba:  'RU', gu:  'IN', bsk: 'PK',
      gn:  'PY', pms: 'IT', yo:  'NG', fa:  'IR', qu:  'PE',
      crh: 'UA', wo:  'SN', kok: 'IN', lad: 'IL', lv:  'LV',
      rm:  'CH', ab:  'GE', am:  'ET', as:  'IN', sd:  'PK',
      vec: 'IT', fur: 'IT', xh:  'ZA', ase: 'US', zu:  'ZA',
      mni: 'IN', oc:  'FR', ti:  'ER', sw:  'TZ', my:  'MM',
      tt:  'RU', srm: 'SR', ha:  'NG', bo:  'CN'
    };
    if (manuallySetCountries[l2.code]) return manuallySetCountries[l2.code];

    // 3) 手動設定がない場合は渡された country 配列を用いて推定
    if (l2.country && l2.country.length) {
      let country;
      if (l2.country.length === 1) {
        country = l2.country[0];
      } else {
        // 言語名先頭2文字と一致する国名を優先
        const pref = l2.country.filter(c => c.name.startsWith(l2.name.slice(0, 2)));
        country = pref.length ? pref[0] :
          l2.country.find(c => c.alpha2Code === l2.code.toUpperCase()) ||
          l2.country[0];
      }
  ...
  // Given a L2 (in iso639-3), return a list of L1s among the commonLangs that have a dictionary for that L2
  supportedL1s(iso639_3, preferredL1Code = null) {
    let englishLanguage = this.getSmart("eng");
    let supportedL1s = this.l1s.filter((language) => {
      const isCommonLanguage = this.commonLangs.includes(language.code);
      const hasDictionary = language.dictionaries?.[iso639_3];
      const hasTranslatedDictionary =
        LANGS_WITH_AZURE_TRANSLATE.includes(language.code) &&
        englishLanguage.dictionaries[iso639_3];
      return isCommonLanguage && (hasDictionary || hasTranslatedDictionary);
    });
    // Sometimes we need to find the most preferrable language, e.g. in the 'stats' component, when a user clicks on a language, we should default to an L1.
    // So we need to sort this by popularity and user's browser setting (passed in via preferredL1Code
    const codesByPopularity =
      "en zh ko it es fr ja de tr ru nl sv pl no id th ca pt vi he el uk cs ar".split(
        " "
      );
    let preferredCodes = [preferredL1Code, ...codesByPopularity.filter(code => code !== preferredL1Code)];
    // Create a map for efficient index lookup
    const codeIndices = new Map(
      preferredCodes.map((code, index) => [code, index])
    );

    const sortedL1s = supportedL1s.sort((a, b) => {
      const aIndex = codeIndices.get(a.code);
      const bIndex = codeIndices.get(b.code);
      
      const aPopularity = aIndex !== undefined ? aIndex : Infinity;
      const bPopularity = bIndex !== undefined ? bIndex : Infinity;
  ...
  getLocaleDescription(locale) {
    ...
    language = this.getSmart(langCode);
    if (countryCodeOrScript) {
      ...
      if (!script) {
        country = this.countryFromCode(countryCodeOrScript);
      }
    }
