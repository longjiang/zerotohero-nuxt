...
export default {
  ...
  assignDictionaries(
    ...
    // Loop through the default wiktionary languages (data.wiktionary_langs), i.e. 'eng' and 'kor'
    for (let l1 in data.wiktionary_langs) {
      ...
      let all_wiktionary_langs = wiktionary_langs.concat(
        wiktionary_mapped_langs
      );
  ...
  getFeatures({ l1, l2 }, browser) {
    ...
    let features =
      l1.features && l1.features[l2["iso639-3"]]
        ...
        : [];
    if (
      l1.dictionaries?.[l2["iso639-3"] || l2["glottologId"]] || LANGS_WITH_AZURE_TRANSLATE.includes(l1.code)
    ) {
  ...
  supportedL1s(iso639_3, preferredL1Code = null) {
    let supportedL1s = this.l1s.filter(
      (language) =>
        this.commonLangs.includes(language.code) &&
        language.dictionaries?.[iso639_3]
    );
    // Sort the supportedL1s based on their 'code' property according to the list 'en zh ko it es fr ja de tr ru nl sv pl no id th ca pt vi he el uk cs ar' (in that order), then others
    let sortedSupportedL1s = [];
    let sortedCodes =
      "en zh ko it es fr ja de tr ru nl sv pl no id th ca pt vi he el uk cs ar".split(
        " "
      );
    for (let code of sortedCodes) {
      let l1 = supportedL1s.find((l) => l.code === code);
      if (l1) sortedSupportedL1s.push(l1);
    }
    for (let l1 of supportedL1s) {
      if (!sortedCodes.includes(l1.code)) sortedSupportedL1s.push(l1);
    }
    // Make sure that the preferred L1 bubbles up to the top
    if (preferredL1Code) {
      let preferredL1Index = sortedSupportedL1s.findIndex(
        (l) => l.code === preferredL1Code
      );
      if (preferredL1Index > -1) {
        let preferredL1 = sortedSupportedL1s.splice(preferredL1Index, 1)[0];
        sortedSupportedL1s.unshift(preferredL1);
      }
    }
    return sortedSupportedL1s;
  },
};
