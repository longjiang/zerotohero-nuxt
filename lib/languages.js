...
export default {
  ...
  async load(limitToCodes = undefined) {
    ...
    let countries = await this.loadCSVFile(`${server}/data/languages/countries.csv.txt`)
    countries = countries.map((row) => {
      row.languages = row.languages ? row.languages.split(",") : [];
      return row;
    });
    let languages = await this.loadCSVFile(`${server}/data/languages/languages.csv.txt`)
    ...
    let omniglot = await this.loadCSVFile(`${server}/data/languages/omniglot.csv.txt`)
    let dictionaries = this.loadCSVString(data.dictionaries)
    let scripts = await this.loadCSVFile(`${server}/data/languages/scripts.csv.txt`)
