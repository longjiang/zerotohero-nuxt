...
export default {
  ...
  liveTVLangs:
    "amh ara aze bak ben bos bul cat ces cmn cnr dan deu ell eng est fas fra fry glg heb hin hrv hun hye iku ind isl ita jpn kan kat kaz kin kor kur lao lav lit ltz mal mkd mlt mri nan nep nld nor pan pol por pus ron rus sin slk slv som spa sqi srp swe tam tel tgl tha tur ukr urd vie yue zho".split(
      " "
    ),
  contentLangs:
  ...
  async loadFull(limitToCodes = undefined) {
    if (!this.fullyLoaded) {
      ...
      let server = typeof window === "undefined" ? process.env.baseUrl : window.location.origin; // Use local (baseUrl defined in nuxt.config.js)
      let locales = this.loadCSVString(data.locales);
      let hours = this.loadCSVString(data.hours);
        ...
        if (l1.country && l1.country[0]) {
            ...
            for (let country of l1.country) {
                ...
                if (country.languages && country.languages[0]) {
                    ...
                    hostCountryLocales.push(primaryLanguageCode, ...primaryLanguageLocales);
                }
  ...
  supportedL1s(iso639_3, preferredL1Code = null) {
    ...
    let sortedCodes = "en zh ko it es fr ja de tr ru nl sv pl no id th ca pt vi he el uk cs ar".split(
      " "
    );
