...
export default {
  ...
  async load() {
    ...
    let languages = await this.loadCSVFile(`${server}/data/languages/common-languages.csv.txt`)
    this.l1s = this.constructL1Data(languages, { features, locales, hours, wiktionary_mapped_langs, dictionaries })
    return this
  },
  async loadFull(limitToCodes = undefined) {
    ...
    let locales = this.loadCSVString(data.locales)
    let hours = this.loadCSVString(data.hours)
    let [countries, languages, omniglot, scripts] = await Promise.all([this.loadCSVFile(`${server}/data/languages/countries.csv.txt`),
    ...
    for (let script of scripts) {
      if (!Array.isArray(this.scriptsIndex[script.lang])) this.scriptsIndex[script.lang] = []
  ...
  constructL1Data(languages, { features = [], locales = [], scripts = [], hours = [], countries = [], wiktionary_mapped_langs = [], dictionaries = [], omniglot = [] } = {}) {
    ...
    let l1s = languages.map(language => {
      ...
      obj.country = countryCodes.map(alpha2Code => countryIndex[alpha2Code]).filter(alpha2Code => alpha2Code)
      return obj
    })
  ...
  exportCSV() {
    ...
    let languages = this.l1s.filter(obj => this.contentLangs.includes(obj['iso639-3']))
    let csv = Papa.unparse(languages.map(item => {
      let obj = Object.assign({}, item)
      delete obj.published
      if (obj.country) obj.country = obj.country.map(c => c.alpha2Code).join(' ')
  ...
  augmentLanguage(l1, { hours = [] } = {}) {
    ...
    l1.han = ['cdo', 'cjy', 'cmn', 'cnp', 'cpx', 'csp', 'czo', 'hak', 'hsn', 'ltc', 'lzh', 'mnp', 'nan', 'och', 'wuu', 'yue', 'zho'].includes(l1['iso639-3']) || ['leiz1236', 'hain1238'].includes(l1.glottologId)
    l1.continua = l1.han || ['th', 'lo', 'ja', 'km', 'ryu', 'bo', 'my', 'dz', 'soa'].includes(l1.code)
    l1.scripts = this.scriptsIndex[l1.code] || []
      ...
      if (l1.country && l1.country[0]) {
        for (let country of l1.country) {
          if (country.languages && country.languages[0]) {
            ...
            hostCountryLocales = hostCountryLocales.concat([countryPrimaryLanguageCode, ...countryPrimaryLanguageLocales])
          }
  ...
  getFeatures({ l1, l2 }, browser) {
    ...
    if (this.featureCache[l2['iso639-3']]) {
      if (!this.featureCache[l2['iso639-3']].includes('speech') && this.checkSpeech(l2, browser)) this.featureCache[l2['iso639-3']].push('speech')
    ...
    if (!features.includes('speech') && this.checkSpeech(l2, browser)) features.push('speech')
  ...
  countryCode(l2) {
    ...
    if (l2.country && l2.country[0]) {
      ...
      if (countries.length === 1) country = countries[0]
        ...
        let prefCountries = l2.country.filter(c => c.name.startsWith(l2.name.slice(0, 2)))
