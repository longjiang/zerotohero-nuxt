...
export const mutations = {
  SET_CHANNELS(state, { channels, l2_id }) {
    ...
    state.channels = state.channels.filter((channel, index, self) =>
...
export const actions = {
  async fetchChannelsByLanguage({ commit, state }, l2_id) {
    ...
    try {
      const channels = await this.$directus.getData(`items/youtube_channels`, { 'filter[l2][eq]': l2_id, 'sort': '-subscribers', 'limit': 100 });
