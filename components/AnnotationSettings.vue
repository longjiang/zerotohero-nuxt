<template>
  <div :class="`annotation-settings annotation-settings-${variant}`">
    ...
    <div v-if="variant === 'page'">
      <div class="mt-3">
        <b-form-checkbox v-if="['ko', 'vi'].includes($l2.code)" class="mb-2" v-model="showByeonggi">
          <span v-if="$l2.code === 'ko'">
            Show hanja next to hanguel (
            <a href="https://ko.wikipedia.org/wiki/%ED%95%9C%EA%B8%80%EC%A0%84%EC%9A%A9%EA%B3%BC_%EA%B5%AD%ED%95%9C%EB%AC%B8%ED%98%BC%EC%9A%A9#%ED%95%9C%EC%9E%90_%EB%B3%91%EA%B8%B0"
              target="_blank">
              병기, 倂記,
              <em>byeonggi</em>
            </a>
          ...
          <span v-if="$l2.code === 'vi'">
            Show
            <a href="https://en.wikipedia.org/wiki/History_of_writing_in_Vietnam#Ch%E1%BB%AF_H%C3%A1n" target="_blank">
              Hán tự
            </a>
        ...
        <template v-if="$hasFeature('transliteration')">
          ...
          <b-form-checkbox class="mb-2" v-if="
        ...
        <b-button-group v-if="$l2.han" class="d-block mb-2">
          <b-button :variant="!useTraditional ? 'secondary' : 'outline-secondary'" @click="useTraditional = false">
            Use Simplified
          </b-button>
          <b-button :variant="useTraditional ? 'secondary' : 'outline-secondary'" @click="useTraditional = true">
            Use Traditional
          </b-button>
        </b-button-group>
        <b-button-group class="d-block">
          <b-button :variant="!useSerif ? 'secondary' : 'outline-secondary'" @click="useSerif = false">
            Use Sans-Serif
          </b-button>
          <b-button :variant="useSerif ? 'secondary' : 'outline-secondary'" style="font-family: serif"
            @click="useSerif = true">
            Use Serif
          </b-button>
        </b-button-group>
